...
function Wedding() {
    ...
    const newdata = data.filter(e => e.category === "wedding")
    console.log(newdata)
    console.log(data)
                ...
                <div className='grid lg:grid-cols-3 md:grid-cols-2 sm:grid-cols-1 gap-4 w-11/12 mx-auto '>
                    {newdata.map(item => (
                        <div className='flex flex-col rounded-md text-black font-semibold' style={{
                            ...
                            <h1> 📍 {item.details}</h1>
                            <div className='items-center flex flex-col'>
                                <button onClick={() => navigate("/booking", { state: { item } })} className='py-2 bg-slate-300 text-2xl rounded-xl w-fit px-10 my-2'>View in Details</button>
                            </div>
                        </div>
                    ))}
                </div>
